...
--
-- The eight arguments are the same, in the same order, as designer-generate.vbs.
-- Every path is an absolute POSIX path. A geobase or a ribbon the run does
-- not use arrives as an empty string.
--
...
on run argv
	if (count of argv) < 8 then
		return "ERROR 1: designer-generate.applescript takes 8 arguments."
	end if
